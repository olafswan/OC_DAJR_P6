...
class previousLightboxModel extends LightboxModel {
  constructor(data, type, url) {
    // 3 parametres :
    // data = le tableau MediaData,
    // type = l'array des url,
  ...
  get title() {
    ...
    for (const media of this._MediaData) {
      ...
      // console.log("C: splited URL", this._currentUrl.split("media/").pop());
      if (
        media._image === this.currentUrl.split("media/").pop() ||
        media._video === this.currentUrl.split("media/").pop()
      ) {
        return media._title;
      }
    }
  ...
  get previousUrl() {
    // cherche l'url précédent dans la liste d'url
    const currentIndex = this._urlList.indexOf(this.currentUrl);
    // cas ou le media est le premier de la liste
    if (currentIndex === 0) {
      return this._urlList[this._urlList.length - 1];
    } else {
...
class nextLightboxModel extends LightboxModel {
  constructor(data, type, url) {
    // 3 parametres :
    // data = le tableau MediaData,
    // type = l'array des url,
